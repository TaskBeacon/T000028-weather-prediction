...
def _summarize_trials(trials: list[dict], fallback_score: int = 0) -> dict[str, float | int]:
    ...
    accuracy = sum(1 for value in correct_values if _as_bool(value)) / len(correct_values) if correct_values else 0.0

    rt_values = [_decision_rt_s(row) for row in trials]
    ...
    sun_prediction_rate = (
        sum(1 for value in predicted_values if value == "sun") / len(predicted_values) if predicted_values else 0.0
    )
...
def run(options: TaskRunOptions):
    """Run Weather Prediction in human/qa/sim mode with one auditable flow."""
